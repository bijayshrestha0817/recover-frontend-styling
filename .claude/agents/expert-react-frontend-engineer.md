...
## This Project (read before generating code)

This repo (`recover`) is the **Next.js 16 (App Router) / React 19.2 / TypeScript** frontend
for a Student Management API. Match the existing stack and conventions — do not introduce
Vite, Redux, or new libraries without being asked.

- **UI:** Mantine v9 (`@mantine/core`, `@mantine/form`, `@mantine/hooks`); some Radix/shadcn + Tailwind. Toasts via `sonner`.
- **Server state:** TanStack Query v5. List queries are keyed `["<entity>", page]`; mutations refresh via `invalidateQueries({ queryKey: ["<entity>"] })` (prefix match covers the paged key). Don't write optimistic `setQueryData` from modals — they lack the `page` and the cache holds the paginated envelope, not an array.
- **HTTP:** feature services use `wretch`; only `AuthContext` uses `axios` (with refresh-on-401). Errors normalized through `handleApi` (`src/lib/error`) → `NormalizedApiError`; surface `.message` as a toast.
- **API envelope:** most endpoints return `ApiResponse<T>` = `{ data, message, success, errors, code, status }` (`src/types/IApiResponse.ts`); consume via `.data`. A few (JWT token endpoints) are raw. List endpoints return **404 with an envelope when empty** — services catch that and return an empty page.
- **Layout:** feature-sliced under `src/features/<feature>/{components,hooks,services,types}`; routes under `src/app/`. Auth tokens (`access_token`/`refresh_token`) live in cookies via `js-cookie`.
- **Verify:** `npx tsc --noEmit` (must be exit 0) and `bun run lint` (Biome; `noUnusedVariables` is an error). The husky pre-commit hook runs `bun run check && format && lint`.
- **Reality over hype:** prefer the patterns already in the codebase. Server Components / Actions / `use()` only where they actually fit App Router usage here — most CRUD UI is client components with TanStack Query.
...
function UserProfile({ userPromise }: { userPromise: Promise<IUser> }) {
...
async function createPost(prevState: IFormState, formData: FormData): Promise<IFormState> {
...
async function sendMessage(text: string): Promise<IMessage> {
...
export function MessageList({ initialMessages }: { initialMessages: IMessage[] }) {
  const [messages, setMessages] = useState<IMessage[]>(initialMessages);
  const [optimisticMessages, addOptimisticMessage] = useOptimistic(messages, (state, newMessage: IMessage) => [...state, newMessage]);
...
export function ChatRoom({ roomId, theme }: IChatProps) {
...
export function useFetch<T>(url: string): IUseFetchResult<T> {
...
function UserList() {
  const { data, loading, error } = useFetch<IUser[]>("https://api.example.com/users");
...
function CustomInput({ placeholder, ref }: IInputProps) {
...
function SearchResults({ query }: ISearchResultsProps) {
